    ...
    def _train(self):
        ...
        for i, (images, targets) in tqdm(
            ...
            labels_neg = (targets.unsqueeze(-1).repeat(1, self._ln_neg)
                          + torch.LongTensor(len(targets), self._ln_neg).to(self._device).random_(1,
            ...
            loss_neg = self._criterion_nll(s_neg.repeat(self._ln_neg, 1), labels_neg.t().contiguous().view(-1)) * float(
                (labels_neg >= 0).sum())
