...
class FER2013Trainer(Trainer):
    ...
    def __init__(self, model, train_set, val_set, test_set, configs):
        ...
        self._ln_neg = self._configs["ln_neg"]
        self._ln_neg_min = self._configs["ln_neg_min"]
        self._lambda = 1
        self._num_classes = self._configs["num_classes"]
        ...
        if self._configs["weighted_loss"] == 0:
            ...
            self._criterion_nll = nn.NLLLoss().to(self._device)
        else:
            self._criterion = nn.CrossEntropyLoss(class_weights).to(self._device)
            self._criterion_nll = nn.NLLLoss(class_weights).to(self._device)
        ...
        self._scheduler = CosineAnnealingLR(
            ...
            T_max=self._configs['max_epoch_num']
        )
    ...
    def _train(self):
        ...
        for i, (images, targets) in tqdm(
            ...
            labels_neg = (targets.unsqueeze(-1).repeat(1, self._ln_neg)
                          + torch.LongTensor(len(targets), self._ln_neg).to(self._device).random_(
                        1, self._num_classes)) % self._num_classes

            # compute output, measure accuracy and record loss
            outputs = self._model(images)

            s_neg = torch.log(torch.clamp(1. - F.softmax(outputs, -1), min=1e-5, max=1.))
            # s_neg *= weight[labels].unsqueeze(-1).expand(s_neg.size()).cuda()
            loss_neg = self._criterion_nll(
                s_neg.repeat(self._ln_neg, 1), labels_neg.t().contiguous().view(-1)) * float(
                (labels_neg >= 0).sum())

            loss_nl = loss_neg / (float((targets >= 0).sum()) + float((labels_neg[:, 0] >= 0).sum()))

            loss_pl = self._criterion(outputs, targets)
    ...
    def _update_training_state(self):
        ...
        # self._lambda = self._scheduler.get_last_lr()[0] / self._lr

        self._ln_neg = max(
            math.ceil(self._configs["ln_neg"] - (self._configs["ln_neg"] - self._ln_neg_min) /
                      self._max_epoch_num * self._current_epoch_num), self._ln_neg_min)
        """self._ln_neg = math.ceil(
            max(
                self._ln_neg_min + (self._configs["ln_neg"] - self._ln_neg_min)
                * self._scheduler.get_last_lr()[0] / self._lr
                , self._ln_neg_min)
        )"""
    ...
    def _logging(self):
        ...
        message = "\nE{:03d}  {:.3f}/{:.3f}/{:.3f} {:.3f}/{:.3f}/{:.3f} | p{:02d} lr: {:.4e} ln: {:02d} Time {}\n".format(
